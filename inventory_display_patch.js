(function(){
  ...
  function normName(v){ return String(v||'').trim().toUpperCase().replace(/[-_#]+/g,' ').replace(/\s+/g,' '); }
  ...
  function renderAggregatedInventory(){
    ...
    for(const b of bales){
      ...
      const season=String(val(shipment,'season','season')||'غير محدد');
      const nameEn=String(val(b,'nameEn','name_en')||'').trim();
      const nameAr=String(val(b,'nameAr','name_ar')||'').trim();
      const grade=String(val(b,'grade','grade')||'').trim();
      ...
      const key=[normName(nameEn)||normName(nameAr),grade.toUpperCase(),weight,season,br.id].join('|');
      let g=groups.get(key);
      if(!g){ g={nameEn,nameAr,grade,weight,season,branch:br,qty:0,buySum:0,landedSum:0}; groups.set(key,g); }
      g.qty+=1; g.buySum+=buy; g.landedSum+=landed;
    }
    ...
    if(table){
      ...
      if(hr && !hr.querySelector('[data-qty-head]')){
        const th=document.createElement('th'); th.dataset.qtyHead='1'; th.textContent='الكمية';
    ...
    body.innerHTML=rows.map(g=>{
      const avgBuy=g.qty?g.buySum/g.qty:0, avgLanded=g.qty?g.landedSum/g.qty:0;
